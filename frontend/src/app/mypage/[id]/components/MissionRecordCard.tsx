...
const MissionRecordCard: React.FC<MissionRecordDataProps> = ({missionRecord, missionRecords, setMissionRecords}) => {
  ...
  return (
    <div id={`record${missionRecord.id}`} className="max-w-sm lg:max-w-xl p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 mb-3 mx-auto w-full">
      ...
      <h1 className={`text-xl font-bold my-4 ${missionRecord.result === "達成" ? "text-blue-500" : "text-red-500"}`}>{missionRecord.title}</h1>
      <div>
        <label htmlFor={`comment${missionRecord.id}`} className="text-lg mt-3 mb-1">コメント</label>
        <textarea id={`comment${missionRecord.id}`} onChange={(e) => {setComment(e.target.value)}} value={comment} className="text-lg w-full border-4" />
      ...
      <div className="flex justify-end items-center gap-3 mt-1">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5 mt-2 text-red-500 shadow-md active:shadow-sm" onClick={deleteConfirm}>
